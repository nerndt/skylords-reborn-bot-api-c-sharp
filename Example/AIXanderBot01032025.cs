...
namespace Bots
{
    ...
    public class AIXanderBot01032025 : IAspWrapperImpl
    {
        ...
        // If same enemy and (nearestOpponentSquadDistance < nearestOpponentSquadDistancePrevious) then keep chasing; else attack nearest orb
        float nearestOpponentSquadDistancePrevious = float.MaxValue; 
        ...
        Dictionary<int, int> playableCards = null;
        ...
        private Command? SpawnUnit(float myPower, Position2D pos, byte cardPos, int unitPower, uint tick, ref float powerRemaining)
        {
            ...
            if (cardChargesMade[cardPosition].Count >= cardMaxCharges[cardPosition])
            {
            ...
            if (myPower >= unitPower && botState.canPlayCardAt < tick)
            {
                ...
                // NGE01022025 cardChargesMade[cardPosition] += 1;
                string message = string.Format("CommandProduceSquad CardPosition:{0} UnitPower:{1} Charges:{2}:{3}", cardPosition, unitPower, cardChargesMade[cardPosition], cardMaxCharges[cardPosition]);
        ...
        public Dictionary<int, int> GetPlayableCards(List<TokenSlot> orbs)
        {
            Dictionary<int, int> dictPlayableCards = new Dictionary<int, int>();
            if (myCurrentDeckOfficialCardIds != null)
            {
                int totalOrbs = orbs.Count();
                int orbsFire = 0;
                int orbsShadow = 0;
                int orbsNature = 0;
                int orbsFrost = 0;
                ...
                { 
                    if (orb.Color == OrbColor.Fire)
                    {
                        orbsFire++;
                    }
                    else if (orb.Color == OrbColor.Nature)
                    {
                        orbsNature++;
                    }
                    else if (orb.Color == OrbColor.Frost)
                    {
                        orbsFrost++;
                    }
                    else if (orb.Color == OrbColor.Shadow)
                    {
                        orbsShadow++;
                    }
                }
                ...
                for (int c = 0; c < myCurrentDeckOfficialCardIds.Ids.Length; c++)
                {
                    ...
                    if (unitOfficialCardId != 0) // Not a card
                    {
                        ...
                        if (card != null)
                        {
                            if (card.orbsTotal <= totalOrbs) // // 0 - Unit, 1 - Building, 2 - Spell
                            {
                                if (card.orbsFire <= orbsFire && card.orbsNature <= orbsNature &&
                                    card.orbsFrost <= orbsFrost && card.orbsShadow <= orbsShadow &&
                                    card.orbsFireFrost <= (orbsFire + orbsFrost) && card.orbsFireNature <= (orbsFire + orbsNature) &&
                                    card.orbsFireShadow <= (orbsFire + orbsShadow) && card.orbsNatureFrost <= (orbsNature + orbsFrost) &&
                                    card.orbsShadowFrost <= (orbsShadow + orbsFrost) && card.orbsShadowNature <= (orbsShadow + orbsNature) &&
                                    card.orbsNeutral <= (orbsFire + orbsNature + orbsFrost + orbsShadow))
                                {
                                    dictPlayableCards[c] = card.type;
                                }
                            }
                        }
                    }
...
#pragma warning restore CS8602
            ...
            for (int i = 0; i < myCurrentDeckOfficialCardIds.Ids.Length; i++)
            {
                ...
                cardMaxCharges.Add(GetCardMaxCharges(i));
                cardChargesMade.Add(new Dictionary<int, int>());
            }
            ...
            unitPower = GetCardPowerCost(0); // Get power cost of first unit in deck
        }

        public int GetCardMaxCharges(int cardPos)
        {
            int maxCharges = 0;
            if (myCurrentDeckOfficialCardIds != null && cardsSMJ != null)
            {
                int unitOfficialCardId = myCurrentDeckOfficialCardIds.Ids[cardPos]; // Starting unit for AI
                if (unitOfficialCardId != 0) // Not a card
                {
                    SMJCard? card = GetCardFromOfficialCardId(cardsSMJ, unitOfficialCardId);
                    if (card != null)
                    {
                        maxCharges = card.maxCharges;

                        //if (botState.isGameStart == true)
                        //{
                        //    Console.WriteLine("Found Card Info:{0}", card.cardName);
                        //    Console.WriteLine("Max Charges:{0} Unit Power:{1} Orbs:{2} Wells{3}", (int)myPower, maxCharges, myOrbs.Count(), myWells.Count());
                        //}
                    }
                    else
                    {
                        Console.WriteLine("Card Not Found:ID{0}", unitOfficialCardId); // (int)SkylordsRebornAPI.Cardbase.CardTemplate.NomadANature);
                    }
                }
            }
            return maxCharges;
        ...
        public int GetCardPowerCost(int cardPos)
        {
            int unitPower = 0;
            if (myCurrentDeckOfficialCardIds != null && cardsSMJ != null)
            {
        ...
        public Command[] Tick(GameState state)
        {
            ...
            bool showTickState = false; // NGE01022025
            if (currentDeck != null)
            {
                ...
                if (botState.canPlayCardAt == uint.MaxValue && tempCommands.Length == 0)
                {
                    ...
                    if (showTickState == true)
                    {
                        ConsoleWriteLine(true, $"Tick {state.CurrentTick} commands: {tempCommands.Length} myPower: {(int)myPower}");
                    }
...
#pragma warning restore CS8602
        ...
        public bool IsEnemyNearBase(float distanceFromBase, out List<Squad>? enemySquad)
        {
            ...
            if (myOrbs.Count > 0)
            {
                ...
                {
                    Position2D orbPos = orb.Entity.Position.To2D();
                    try
                    {
                        lock (enemySquadsLock)
                        {
...
#pragma warning restore CS8602
        ...
        */
        ...
        private Command? SpawnArcherOnBarrier(float myPower, Position2D pos, EntityId barrierModuleId, uint tick, ref float powerRemaining)
        {
            ...
            if (archerCardPositions != null && myCurrentDeckOfficialCardIds != null)
            {
                byte cardPosition = (byte)archerCardPositions[0]; // Get first archer for now
                int unitOfficialCardId = myCurrentDeckOfficialCardIds.Ids[cardPosition]; // Starting unit for AI
                if (unitOfficialCardId != 0) // Not a card
        ...
        */
        ...
        private Command? GetNearestEnemyOrb(List<TokenSlot> orbs, List<Squad> squads, out float nearestOrbDistanceSq)
        {
            Command? command = null;
            {
                nearestOrbDistanceSq = float.MaxValue;
                float d = float.MaxValue;
                Position2D nearestOrbPosition = Position2DExt.Zero();
                Position2D nearestSquadPosition = Position2DExt.Zero();
                EntityId nearestOrb = new EntityId(0u);
                EntityId nearestSquad = new EntityId(0u);
                foreach (var squad in squads)
                {
                    Position2D posSquad = squad.Entity.Position.To2D();

                    foreach (var orb in orbs)
                    {
                        Position2D posOrb = orb.Entity.Position.To2D();
                        if (squad.Entity.PlayerEntityId == null)
                            continue;
                        d = DistanceSquared(posOrb, squad.Entity.Position);

                        if (d < nearestOrbDistanceSq) // Closest orb from list
                        {
                            nearestOrb = orb.Entity.Id;
                            nearestSquad = squad.Entity.Id;
                            nearestOrbDistanceSq = d;
                            nearestOrbPosition = orb.Entity.Position.To2D();
                            nearestSquadPosition = squad.Entity.Position.To2D();
                        }
                    }
                }

                // Have the squad attack the orb
                if (nearestOrb.V != 0 && nearestSquad.V != 0)
                {
                    ...
                    return new CommandGroupAttack { Squads = new[] { nearestSquad }, TargetEntityId = nearestOrb, ForceAttack = true }; // NGE04292024 ForceAttack = false
                }
            }
            return command;
        }

        private Command? GetNearestEnemyOrb(TokenSlot orb, List<Squad> squads, out float nearestOrbDistanceSq)
        {
            Command? command = null;
            {
                nearestOrbDistanceSq = 0f;
                float d = float.MaxValue;
                Position2D nearestOrbPosition = Position2DExt.Zero();
                Position2D nearestSquadPosition = Position2DExt.Zero();
                EntityId nearestOrb = new EntityId(0u);
                ...
                foreach (var squad in squads)
                {
                    ...
                    if (nearestOrbDistanceSq < d) // Closest orb from list
                    {
                        nearestOrb = orb.Entity.Id;
                        nearestSquad = squad.Entity.Id;
                        nearestOrbDistanceSq = d;
        ...
        private Command? GetNearestEnemy(List<Squad> enemySquads, List<Squad> squads, out float nearestOrbDistanceSq)
        {
            Command? command = null;
            {
                nearestOrbDistanceSq = float.MaxValue;
                float d = float.MaxValue;
                Position2D nearestEnemyPosition = Position2DExt.Zero();
                Position2D nearestSquadPosition = Position2DExt.Zero();
                EntityId nearestEnemy = new EntityId(0u);
                EntityId nearestSquad = new EntityId(0u);
                foreach (var squad in squads)
                {
                    Position2D posSquad = squad.Entity.Position.To2D();
                    ...
                    {
                        Position2D posOrb = enemySquad.Entity.Position.To2D();
                        if (squad.Entity.PlayerEntityId == null)
                            continue;
                        d = DistanceSquared(posOrb, squad.Entity.Position);
                        ...
                        if (d < nearestOrbDistanceSq) // Closest orb from list
                        {
                            nearestEnemy = enemySquad.Entity.Id;
                            nearestSquad = squad.Entity.Id;
                            nearestOrbDistanceSq = d;
                            nearestEnemyPosition = enemySquad.Entity.Position.To2D();
                            nearestSquadPosition = squad.Entity.Position.To2D();
                        }
                    }
                }

                // Have the squad attack the enemy
                if (nearestEnemy.V != 0 && nearestSquad.V != 0)
                {
                    string message = string.Format("Go to nearest EnemyId:{0} at pos:{1},{2}", nearestEnemy, (int)nearestEnemyPosition.X, (int)nearestEnemyPosition.Y);
                    return new CommandGroupAttack { Squads = new[] { nearestSquad }, TargetEntityId = nearestEnemy, ForceAttack = true }; // NGE04292024 ForceAttack = false
                }
            }
            return command;
        }

        private Command? GetNearestEnemy(List<Squad> orbs, Squad squad, out float nearestOrbDistanceSq)
        {
            Command? command = null;
            {
                nearestOrbDistanceSq = 0f;
                ...
                foreach (var orb in orbs)
                {
                    ...
                    if (nearestOrbDistanceSq < d) // Closest orb from list
                    {
                        nearestEnemy = orb.Entity.Id;
                        nearestSquad = squad.Entity.Id;
                        nearestOrbDistanceSq = d;
                        nearestEnemyPosition = orb.Entity.Position.To2D();
                        nearestSquadPosition = squad.Entity.Position.To2D();
                    }
                }
            ...
            switch (currentDeck.Name)
            {
                ...
                case "InfusedFlame":
                    ...
                    foreach (var s in state.Entities.Squads)
                    {
                        if (s.Entity.PlayerEntityId == botState.myId)
                        {
                            ...
                            if (squadHealth != 0)
                            {
                                ...
                                mySquads.Add(s);
                                // SMJCard? card = GetCardFromCardId(s.CardId);

                                int position = GetCardPositionFromCardID(s.CardId);
                                if (position >= 0)
                                {
                        ...
                        else if (s.Entity.PlayerEntityId != null && botState.oponents.Contains(s.Entity.PlayerEntityId))
                        {
                            ...
                            if (squadHealth != 0)
                            {
                                ...
                                enemySquads.Add(s);
                                // Debug.WriteLine($"Enemy squad:{s.Entity.Id} has hp:{squadHealth}");
                            }
                        }
                    }
                    ...
                    if (currentTick.V % defaultTickUpdateRate == 0) // defaultTickUpdateRate can slow down update rate - default tick rate is 1/10 second
                    {
                        ...
                        if (myOrbs != null && myOrbs.Count > 0 && myOrbs[0].Color != OrbColor.Starting)
                        {
                        ...
                        #endregion See if enemy is building an orb
                        ...
                        // NGE11022024 
                        if (myOrbs != null && myOrbs.Count > 0) // NGE12292-24 
                        {
                        ...
                        // Squad? nearestEnemy = null;
                        if (enemyBuildingOrb == false) // NGE01022025 !!!!!!!
                        {
                        ...
                        if (botState.isGameStart == true || currentTick.V % defaultTickUpdateRate == 0) // Try to do stuff every 0.5 seconds instead of every 1/10 second
                        {
                            ...
                            if (myOrbs.Count > 0 && myPower > unitPower)
                            {
                                ...
                                var targetSquadPrevious = new EntityId(0u);
                                ...
                                string targetType = "";
                                var nearestOrb = new EntityId(0u);
                                Entity nearestEntityOrb = null;
                                ...
                                if ((myAttackSquads != null && myAttackSquads.Count > 0) && (enemyBuildingOrb == true || (enemyOrbCount > 1)))
                                {
                                    ...
                                    else if (attacknearestEnemy != null && enemySquads != null && enemySquads.Count > 0)
                                    {
                                ...
                                if (attackSquadCount < defaultAttackSquads || myPower > maxPowerToDoMore) // NGE08272024!!!!! else if (attackSquadCount < defaultAttackSquads || myPower > maxPowerToDoMore)
                                {
                                    ...
                                    //List<Squad> swiftAttackUnits = new List<Squad>();
                                    if (myAttackSquads != null && myAttackSquads.Count > 0)
                                    {
                                        if ( swiftAttackUnits == null)
                                        {
                                            swiftAttackUnits = new List<Squad>();
                                        }
                                        swiftAttackUnits.Clear();

                                        foreach (var s in myAttackSquads) // if squad is near Entity, attack it
                                        {
                                            if (swiftAttackUnits.Count < totalSwiftAttackUnits && swiftCardPositions != null && swiftCardPositions.Count > 0)
                                            {
                                                if (s.CardId == currentDeck.Cards[(byte)swiftCardPositions[0]])
                                                {
                                                    swiftAttackUnits.Add(s);
                                                }
                                                if (swiftAttackUnits.Count >= totalSwiftAttackUnits)
                                                {
                                        ...
                                        if (armyHasSwiftUnit == false)
                                        {
                                            ...
                                            if (cardChargesMade[cardPos].Count >= cardMaxCharges[cardPos])
                                            {
                                                if (playableCards != null && playableCards.Count > 1)
                                                {
                                                    foreach (KeyValuePair<int, int> kvp in playableCards)
                                ...
                                {
                                    if (myWalls != null && myWalls.Count > 0 && myDefendSquads != null && myDefendSquads.Count() > 0) // Make sure archers are on walls if near them!
                                    {
                                        //archerSquad
                                    }
                                }
                                ...
                                */ // NGE05232024 !!!!!!
                                ...
                                // NGE12292024 float nearestOpponentSquadDistance = float.MaxValue;
                                if (attackClosestSquad == true || (enemyOrbs.Count() == 0 && enemyWells.Count() == 0))
                                {
                                    try
                                    {
                                        lock (enemySquadsLock)
                                        {
                                            nearestOpponentSquadDistancePrevious = nearestOpponentSquadDistance;
                                            foreach (var s in enemySquads) // if squad is near me attack it
                                            {
                                                foreach (Squad squad in myAttackSquads)
                                                {
                                                    ...
                                                    if (squadDistanceToOpponent < nearestOpponentSquadDistance)
                                                    {
                                                        ...
                                                        targetSquadPrevious = targetSquad;
                                            ...
                                            // If same enemy and (nearestOpponentSquadDistance < nearestOpponentSquadDistancePrevious) then keep chasing; else attack nearest orb
                                            if (targetSquadPrevious.V != 0 && targetSquadPrevious == targetSquad && nearestOpponentSquadDistance < nearestOpponentSquadDistancePrevious) // Keep attacking target squad
                                            {
                                                nearestOpponentSquadDistance = float.MaxValue;
                                            }
                                            else // Attack enemy orb
                                            {
                                                if (myAttackSquads != null)
                                                {
                                                    Command? attacknearestEnemyOrb = GetNearestEnemyOrb(enemyOrbs, myAttackSquads, out float nearestOrbDistanceSq);
                                                    if (attacknearestEnemyOrb != null)
                                                    {
                                                        commands.Add(attacknearestEnemyOrb); // NGE12292024!!!!!! 
                                                        return commands.ToArray(); // NGE12292024!!!!!! 
                                                    }
                                                }
                                            }
                                ...
                                */ // NGE05292024 
                                ...
                                // NGE01012025 else
                                // NGE01012025 {
                                // NGE01012025     if (targetEntity == null) // Attack nearest unit or orb
                                // NGE01012025     {
                                // NGE01012025         previousTarget = target;
                                // NGE01012025         target = targetOrb;
                                // NGE01012025         targetEntity = targetEntityOrb;
                                // NGE01012025         targetType = "Orb";
                                // NGE01012025     }
                                // NGE01012025 }

                                if (target.V == 0 || targetEntity == null)
                                {
                                    var orb = nearestOrb; // NGE01022025
                                    // NGE01022025 var orb = enemyOrbs.Where(o => o.Entity.Id.V != 0).FirstOrDefault();
                                    if (orb != null)
                                    {
                                        // NGE01012025
                                        previousTarget = target;
                                        target = targetOrb;
                                        targetEntity = nearestEntityOrb; // NGE01022025 targetEntityOrb;
                                        targetType = "Orb";
                                    ...
                                    // Go attack the opponent's orb unless it is behind a wall; if behind wall, attack wall or something else!
            ...
            Ids = new int[20]
    {
            ...
            Cards = new Api.CardId[20]
     {
        ...
        */
        ...
        /*
        public static void GetRemainingCharges(int CardId, RemainingCharges remainingCharges, SMJCard[]? cardsSMJ, DeckOfficialCardIds cards) // cards = myCurrentDeckOfficialCardIds
        {
            int unitOfficialCardId = cards.Ids[(int)CardId];

            SMJCard? card = GetCardFromOfficialCardId(cardsSMJ, unitOfficialCardId);
            // var result = new (CardId, RemainingCharges)[cards.Ids.Length];
            // for (var i = 0; i < cards.Length; i++)
            {
                // var card = cards[CardId]; // (cards[i].Id);
                var amount = GetPlayCount(card, card.Upgrade);
                uint cooldown = 0, restoreTime = 0;
                switch (card.CardType)
                {
                    case "Building":
                        restoreTime = (uint)(main.GetBuildingById(card.DbEntityId).ProductionTime * 10) / 2;
                        break;
                    case "Squad":
                        restoreTime = (uint)(main.GetSquadById(card.DbEntityId).ProductionTime * 10) / 2;
                        break;
                    case "Spell":
                        var spell = main.GetSpellById(card.DbEntityId);
                        cooldown = (uint)spell.RecastTicks;
                        restoreTime = (uint)Math.Max(spell.ProductionTime * 10 / 2, spell.RecastTicks);
                        break;
                    default:
                        if (cards[i].Id != 0) throw new Exception("Invalid card");
                        break;
                }

                result[i] = (cards[i], new RemainingCharges
                {
                    Amount = RemainingChargesAmount.SomeCharges,
                    Cooldown = cooldown,
        ...
        */
        ...
        public static void UseCharge(ref RemainingCharges charges, uint currentTick)
        {
            switch (charges.Amount)
            {
                case RemainingChargesAmount.SomeCharges:
                    charges.Amount = RemainingChargesAmount.None;
                    break;
                case RemainingChargesAmount.Cooldown:
                    if (charges.Cooldown >= currentTick) throw new Exception("Cooldown");
                    charges.Amount = RemainingChargesAmount.None;
                    break;
                case RemainingChargesAmount.RestoringCharge:
                    if (charges.RestoreTime >= currentTick) throw new Exception("OutOfChargesCooldown");
                    charges.Amount = RemainingChargesAmount.None;
                    break;
            }
        }
        ...
        public class RemainingCharges
        {
            public RemainingChargesAmount Amount { get; set; }
            public uint Cooldown { get; set; }
            public uint RestoreTime { get; set; }
        }

        public enum RemainingChargesAmount
        {
            None,
            SomeCharges,
            Cooldown,
...
public static class Main {
    public static Card GetCardById(int id) => new Card();
    public static Building GetBuildingById(int id) => new Building();
    public static Squad GetSquadById(int id) => new Squad();
    public static Spell GetSpellById(int id) => new Spell();
}
...
public class Spell {
    public int RecastTicks { get; set; }
    public int ProductionTime { get; set; }
}

public class Program {
    public static (CardId, RemainingCharges)[] InitializeDeck(CardId[] cards) {
        var main = Main.GetInstance();
        var result = new (CardId, RemainingCharges)[cards.Length];
        for (var i = 0; i < cards.Length; i++) {
            var card = main.GetCardById(cards[i].Id);
            var amount = GetPlayCount(card, card.Upgrade);
            uint cooldown = 0, restoreTime = 0;
            switch (card.CardType) {
                case "Building":
    ...
    private static int GetPlayCount(Card card, int upgrade) {
        // Method logic here
        return 0;
    }

    private static CardId[] Deck = new CardId[]; // Just for the example
}
